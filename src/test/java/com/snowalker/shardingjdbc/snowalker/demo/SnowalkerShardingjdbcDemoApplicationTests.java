...
import com.snowalker.shardingjdbc.snowalker.demo.service.UoService;
...
//            long orderId = KeyGenerator.getKey();
    ...
    @Test
    public void testInsertUoInfo() {
        for (int i = 0; i < 500; i++) {
//            long orderId = KeyGenerator.getKey();
            long orderId =i;
            UoInfo orderInfo = new UoInfo();
            orderInfo.setOrderId(orderId);
            int result = uoService.addOrder(orderInfo);
            if (1 == result) {
                LOGGER.info("入库成功,orderInfo={}", orderInfo);
            } else {
                LOGGER.info("入库失败,orderInfo={}", orderInfo);
    ...
    @Test
    public void testGetUoInfo(){
        List<UoInfo> list = new ArrayList<>();
        UoInfo orderInfo = new UoInfo();
        orderInfo.setOrderId(33L);
        list = uoService.getUoInfo(orderInfo);
